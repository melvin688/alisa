  ...
  <div class="page-container">
    ...
    <div class="header-actions">
      <div class="header-title">
        <span>{{ $t('takeaway.title') }}</span>
        <el-tag type="info" style="margin-left: 12px;">{{ orderList.length }} {{ $t('takeaway.takeawayOrders') }}</el-tag>
      </div>
    ...
    <div class="orders-grid" v-loading="loading">
      <el-empty 
        ...
        :description="$t('takeaway.noOrders')"
        :image-size="200"
      />
        ...
        <div class="order-card-body">
          ...
          <div class="pickup-info">
            <div class="info-item">
              ...
              <span class="info-label">{{ $t('takeaway.pickupTime') }}:</span>
...
import { getTakeawayOrders, updateOrderStatus, printOrder as printOrderAPI, deleteOrder } from '@/api/orders'
...
const loadOrders = async () => {
  loading.value = true
  try {
    const res = await getTakeawayOrders()
    if (res.success) {
      orderList.value = res.data || []
    }
  } catch (error) {
    console.error('加载自取订单失败:', error)
...
const handlePrintAll = async () => {
  try {
    ...
    ElMessage.success(t('takeaway.printSuccess'))
  } catch (error) {
    if (error !== 'cancel') {
...
const handleClearAll = async () => {
  try {
    ...
    ElMessage.success(t('takeaway.clearSuccess'))
    await loadOrders()
  } catch (error) {
    if (error !== 'cancel') {
